...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    """
    percentage_value = (voltage*current)/theoretical_max_power
    if percentage_value >= 0.8:
        return "green"
    elif 0.6 <= percentage_value <= 0.8:
        return "orange"
    elif  0.3 <= percentage_value <= 0.6:
        return "red"
    return "black"
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    if state < lower_bound:
        return "LOW"
    elif lower_bound <= state <= upper_bound:
        return "NORMAL"
    return "DANGER"

print(fail_safe(10,1000,10000))